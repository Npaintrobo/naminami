...
export function evaluateDesign({ N, pitch, samplesPerWave, thickness, amplitude, rpm }) {
  ...
  const ideal = mapEnvelope(N, samplesPerWave, 0), compliant = mapEnvelope(N, samplesPerWave, 1);
  ...
  const mechanicsPass = slope <= 0.25 && surfaceStrain <= 0.05 && verticalAccelerationG <= 0.8;
  const finiteDiskPass = wavesAcrossDiameter >= 2;
  return {
    N, pitch, samplesPerWave, thickness, amplitude, rpm, wavelength, diameter,
    wavesAcrossDiameter, slope, surfaceStrain, verticalAccelerationG, surfaceSpeed,
...
const prototype = evaluateDesign({ N: 12, pitch: 25, samplesPerWave: 6, thickness: 10, amplitude: 5, rpm: 300 });
console.log('\n比較');
for (const [name, r] of [['現行例', current], ['推奨一次試作', prototype]]) console.log(
