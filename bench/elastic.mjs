...
console.log('■ 連続下面の鉛直変位に対する、2次元無荷重帯の準静的応答。\n');
...
for (const bl of ['fixed', 'free']) {
  for (const [lam, thick, W] of [[100, 6, 3], [100, 6, 5], [150, 6, 5], [100, 10, 5]]) {
    ...
    console.log(`  ${(bl === 'fixed' ? '全下面X固定' : '自由（端部のみ固定）').padEnd(14)} ${String(lam).padStart(4)}  ${String(thick).padStart(5)}  ${String(W).padStart(4)}   ${R(r)}`);
  }
}
...
■ 分かったこと
  ...
  ・連続下面を持つ2次元無荷重帯では、鉛直規定変位だけから上面の面内変位が出る。
  ・全下面のX変位固定では薄板予測の1.3〜1.8倍、端部だけを止めた極限では0.93〜0.99。
    これは同心円溝の実機利得ではなく、2種類の境界条件による上限・下限の候補。

■ ただし注意

  ・同心円溝が拘束するのは**半径方向だけ**。波の進む向きが半径方向と一致する場所では
    X固定、直交する場所では自由境界に近づく可能性があるが、1.0〜1.7という円板内分布を
    この2次元モデルから補間してはいけない。円板・離散ピンは discrete-map.mjs で別に扱う。
